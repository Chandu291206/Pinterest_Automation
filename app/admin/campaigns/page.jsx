...
export default async function CampaignsPage() {
    ...
    try {
        ...
        const [campaignsRes, pinsRes] = await Promise.all([
            supabase.from("campaigns").select("id,name,theme,posts_per_day,interval_hours,status").order("created_at", {
                ascending: false,
            }),
    ...
    return (<div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {dataError ? (<p className="text-sm text-destructive">{dataError}</p>) : (<Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Name</TableHead>
                  <TableHead>Theme</TableHead>
                  <TableHead>Posts/Day</TableHead><TableHead>Interval</TableHead>
              ...
              <TableBody>
                ...
                {campaigns.map((campaign) => (<TableRow key={campaign.id}>
                    ...
                    <TableCell>{campaign.posts_per_day ?? 3}</TableCell><TableCell>{campaign.interval_hours ?? 1}h</TableCell>
                    <TableCell>
