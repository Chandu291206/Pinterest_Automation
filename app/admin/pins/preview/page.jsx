...
export default function PinPreviewPage() {
    ...
    const [imageUrlsRaw, setImageUrlsRaw] = useState("https://m.media-amazon.com/images/I/71GeRoUotCS.jpg\nhttps://m.media-amazon.com/images/I/41vF-oMm5XL.jpg");
    ...
    const parsedUrls = useMemo(() => imageUrlsRaw
        .split("\n")
        .map((url) => url.trim())
        .filter(Boolean), [imageUrlsRaw]);
    useEffect(() => {
        return () => {
            if (previewUrl) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-2">
              <Label>Format</Label>
              <Select value={format} onValueChange={(value) => setFormat(value === "collage" ? "collage" : "single")}>
            ...
            <div className="grid gap-2">
              ...
              <Select value={theme} onValueChange={(value) => setTheme(value || "fitness")}>
                ...
                <SelectContent>
                  {themeOptions.map((item) => (<SelectItem key={item} value={item} className="capitalize">
                      {item}
                    </SelectItem>))}
                </SelectContent>
              </Select>
            ...
            <div className="grid gap-2">
              ...
              <Textarea rows={7} value={imageUrlsRaw} onChange={(event) => setImageUrlsRaw(event.target.value)}/>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            {previewUrl ? (<img src={previewUrl} alt="Pin preview output" className="w-full rounded-md border"/>) : (<div className="flex min-h-[500px] items-center justify-center rounded-md border border-dashed text-sm text-muted-foreground">
