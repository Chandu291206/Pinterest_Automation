...
export default async function DashboardPage() {
    ...
    try {
        ...
        totalImpressions = (pinStatsRes.data ?? []).reduce((sum, pin) => sum + Number(pin.impressions ?? 0), 0);
        totalClicks = (pinStatsRes.data ?? []).reduce((sum, pin) => sum + Number(pin.clicks ?? 0), 0);
        recentPins = (recentPinsRes.data ?? []);
    }
    catch (error) {
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {recentPins.length === 0 ? (<TableRow>
                  <TableCell colSpan={5} className="text-center text-muted-foreground">
                    No pins yet.
                  </TableCell>
                </TableRow>) : null}
              {recentPins.map((pin) => (<TableRow key={pin.id}>
                  <TableCell>
                    {pin.image_url ? (<img src={pin.image_url} alt={pin.title} className="h-14 w-14 rounded-md object-cover"/>) : (<div className="flex h-14 w-14 items-center justify-center rounded-md bg-muted text-xs text-muted-foreground">
                        No image
                      </div>)}
                  </TableCell>
