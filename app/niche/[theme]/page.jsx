...
export default async function NicheThemePage({ params }) {
    const theme = params.theme.toLowerCase();
    if (!allowedThemes.has(theme)) {
        notFound();
    }
    const items = await fetchLatestPostedProducts({ limit: 48, theme });
    return (<main className="min-h-screen bg-white px-4 py-8">
      <div className="mx-auto w-full max-w-6xl space-y-6">
        ...
        {items.length === 0 ? (<p className="text-sm text-gray-500">No posted products found for this theme yet.</p>) : (<section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((item) => (<ProductCard key={item.pinId} item={item}/>))}
          </section>)}
      </div>
    </main>);
}
