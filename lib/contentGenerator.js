...
function normalizeHashtags(input, fallbackTerms) {
...
function safeWords(text, maxWords) {
  const words = text.trim().split(/\s+/).filter(Boolean);
  return words.slice(0, maxWords).join(" ");
}

function safeChars(text, maxChars) {
  const trimmed = text.trim();
  return trimmed.length <= maxChars ? trimmed : `${trimmed.slice(0, maxChars - 3).trim()}...`;
}

function ensureSoftCta(description) {
...
function getLocalClient(baseURL) {
  if (localClient && localClient.baseURL === baseURL) return localClient;
...
async function getRuntimeLlmConfig() {
  const settings = await getSettings(["llm_provider", "openai_model"]);

  const provider = (settings.get("llm_provider") || process.env.LLM_PROVIDER || "openai")
    .toLowerCase()
    .trim();

  const openaiModel = (settings.get("openai_model") || process.env.OPENAI_MODEL || "gpt-4o-mini").trim();
  ...
  if (provider === "local") {
    ...
    if (!baseURL || !model) {
      throw new Error("LOCAL_LLM_URL and LOCAL_LLM_MODEL are required when LLM provider is local.");
    }

    return {
      provider: "local",
      client: getLocalClient(baseURL),
      model,
    };
  }
...
export async function generatePinContent(params) {
  const runtime = await getRuntimeLlmConfig();
  ...
  try {
    ...
    throw new Error(
      `Model ${runtime.model} returned non-JSON output. Choose a model that supports strict JSON responses.`
    );
  }
