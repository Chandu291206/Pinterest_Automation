...
function splitHeadline(headline, maxCharsPerLine = 22, maxLines = 2) {
    const words = headline.trim().split(/\s+/).filter(Boolean);
    if (words.length === 0)
        return ["Shop This Pick"];
    const lines = [];
    let currentLine = "";
...
function buildOverlaySvg(headline, priceBadge, theme) {
    ...
    const headlineStartY = safeHeadlineLines.length === 1 ? headlineBaseY : headlineBaseY - headlineLineSpacing / 2;
...
async function createFallbackTile(width, height) {
    const svg = `
...
function getCollageSlots(count) {
    if (count === 2) {
        return [
...
export async function resizeToFill(buffer, width, height) {
    return sharp(buffer)
        .resize(width, height, {
        fit: "cover",
        position: "centre",
...
export async function createSingleProductPin(params) {
    let productBuffer;
    try {
...
export async function createCollagePin(params) {
    ...
    const resizedBuffers = await Promise.all(productImageUrls.map(async (url, index) => {
    ...
    const withTiles = collageBase.composite(resizedBuffers.map((buffer, index) => ({
        input: buffer,
...
export async function uploadPinImage(buffer, pinId) {
